...
void lcd_printInt(char x, char y, long int n) 
	{
		char str[33];
		CLCD_I2C_SetCursor(&LCD1, x, y);
		sprintf(str,"%i  ",n);
...
void lcd_printFloat(char x, char y, float n) 
	{
		char str[33];
		CLCD_I2C_SetCursor(&LCD1, x, y);
		sprintf(str,"%0.2f  ", n);
...
void lcd_printBit(long int n) 
	{
		if(n>0)		CLCD_I2C_WriteString(&LCD1, "1");
			else CLCD_I2C_WriteString(&LCD1, "0");
	}
